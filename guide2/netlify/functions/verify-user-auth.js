...
if (!admin.apps.length) {
  try {
    ...
    // Try to use the service account key from environment
    if (process.env.FIREBASE_ADMIN_SDK_KEY) {
      try {
        const serviceAccount = JSON.parse(process.env.FIREBASE_ADMIN_SDK_KEY);
        credential = admin.credential.cert(serviceAccount);
        console.log('Using service account credentials from FIREBASE_ADMIN_SDK_KEY');
      } catch (parseError) {
        console.error('Error parsing FIREBASE_ADMIN_SDK_KEY:', parseError);
        throw parseError;
      }
