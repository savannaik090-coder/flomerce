...
- **Subcategories:** Categories support hierarchical parent-child relationships via `parent_id` in the `categories` table. Products store `subcategory_id` referencing a child category. Admin product form shows subcategory picker (with inline creation) when a parent category is selected. Storefront category pages fetch real subcategories and display them as filter options in the filter modal, replacing fake price range filters. Column auto-migration via `ensureProductSubcategoryColumn()`. Category deletion cascades to null both `category_id` and `subcategory_id` on products.
