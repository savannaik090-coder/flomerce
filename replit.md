...
- **Duplicate Subscription Prevention:** Unique index on `subscriptions.razorpay_subscription_id` prevents duplicate records. `activateSubscription()` checks for existing records and either skips (if active) or updates (if non-active) before inserting.
