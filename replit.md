...
- categories, products, product_variants, orders, guest_orders, carts, wishlists, site_customers, site_customer_sessions, customer_addresses, customer_password_resets, customer_email_verifications, coupons, notifications, reviews, page_seo, page_views, site_media, site_usage, activity_log, addresses
...
- **Built-in Analytics:** Lightweight page view tracking via `page_views` table in shard DBs. A `usePageTracker` hook in App.jsx sends beacons on every storefront page navigation (skips admin routes). Tracks: page path, referrer, country (from Cloudflare `CF-IPCountry` header), device type, browser (parsed from UA), and anonymous visitor ID (localStorage). Backend `analytics-worker.js` handles `POST /api/analytics/track` (recording) and `GET /api/analytics/stats?siteId=&period=` (aggregated stats). Admin AnalyticsSection shows: visitors, page views, bounce rate, visitor trends chart, traffic sources, device breakdown, top countries, and top pages — all from real data.
