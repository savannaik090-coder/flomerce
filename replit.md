...
- **Authentication:** Custom JWT-based system for platform users and per-site customer accounts. Site admin access uses a verification-code-based system.
...
- **Storage Usage Tracking:** Per-site D1 (database) and R2 (media) storage tracking across all site-specific operations. D1 tracking covers: products (CRUD), categories (CRUD), orders (create/update), guest orders, customer signups, customer addresses (create/delete), wishlists (add/remove), cart creation, SEO data (site/category/product/page SEO, social tags), and site settings updates. R2 tracking covers: image uploads/deletes and video uploads/deletes via upload-worker. Platform-level tables (users, sessions, subscriptions, payments) are not tracked per-site. Usage stored in `site_usage` / `site_media` tables. Plan limits: Basic 500MB D1/5GB R2, Pro 1.5GB/50GB, Premium 3GB/100GB. Basic/Pro hard-block at limit. Premium users must opt-in to overage charges via billing toggle (`overageEnabled` in site settings JSON). Overage rates: ₹0.75/GB D1, ₹0.015/GB R2. Reconciliation function auto-backfills usage on first request (scans products, categories, orders, guest_orders, site_customers, reviews, coupons, customer_addresses, wishlists, carts, page_seo). Core logic in `backend/utils/usage-tracker.js`. API: `GET /api/usage?siteId=...` (read), `POST /api/usage?siteId=...` (toggle overage). Workers with tracking: products-worker, categories-worker, orders-worker, upload-worker, customer-auth-worker, site-admin-worker, wishlist-worker, cart-worker, sites-worker.
