...
### Internationalization (i18n) — Active language policy (Apr 2026)
- **English-default policy, no auto-detection.** The UI boots in English for every visitor regardless of browser locale, geo-IP, or merchant-configured `content_language`. The user only leaves English by explicitly picking another option from the LanguageSwitcher (which writes `flomerce_lang` + `flomerce_lang_explicit` to localStorage). Implemented in `frontend/src/shared/i18n/init.js`: detection `order: ['localStorage']` only (no `navigator`/`htmlTag`), and any prior implicit `flomerce_lang` value is cleared at boot when the explicit marker is absent. The merchant-content-language auto-switch in `frontend/src/storefront/src/context/SiteContext.jsx` is intentionally a documented no-op for the same reason.
- **Geo-filtered dropdown options.** `GET /api/i18n/geo` reads Cloudflare's `cf-ipcountry` header and returns one of two curated buckets:
  - **India (`IN`)**: `en, hi, ta, te, ml, kn, mr, bn, gu` (9 languages — English, Hindi, Tamil, Telugu, Malayalam, Kannada, Marathi, Bengali, Gujarati)
  - **All other countries**: `en, hi, es, fr, de, it, nl, pt, ja, ko, zh-CN` (11 languages — English + Hindi for diaspora + 9 foreign majors)
  - **Total active set**: 17 unique locales (the union, exposed as `PRESHIPPED` from `init.js`). `INDIA_LANGUAGES` and `FOREIGN_LANGUAGES` constants are also exported.
  - Arabic, Russian, Hebrew, and other previously-allowlisted lazy-gen locales are intentionally NOT in the active dropdown set (RTL/payments/compliance friction). They remain in `SUPPORTED_LOCALES` so any user typing `?lang=ar` directly can still get a translated catalog without the platform spending generation budget on speculative locales.
  - Geo response is `Cache-Control: no-store` for browsers + `CDN-Cache-Control: max-age=60` for Cloudflare edge, with `Vary: cf-ipcountry, Accept-Encoding` so the edge keys per country (prevents an India response leaking to a US visitor).
- **LanguageSwitcher UX.** `frontend/src/shared/i18n/LanguageSwitcher.jsx` fetches geo on mount (module-level promise dedupes multi-instance pages), shows the bucket's native-script labels, and never drops the user's explicit choice from the visible options even if they travel to a different country. Used on platform navbar, storefront navbar, and seller admin per the "all surfaces" decision.
- **Owner translations panel and merchant content-language picker** both derive their option list from the shared `PRESHIPPED` set (`I18nAdminPanel.jsx`, `SiteCreationWizard.jsx` `CONTENT_LANGUAGE_LABELS`), so adding/removing a code in `init.js` automatically flows everywhere.
- **Dev preview proxy.** `run-server.js` forwards `/api/*` from the Express SPA host (:5000) to the wrangler worker (:8000) and injects a default `cf-ipcountry: IN` header so India bucket testing works locally. 30s timeout, returns 502/504 on backend issues. In production the wrangler worker serves SPA + API on a single origin so this proxy is dev-only.
