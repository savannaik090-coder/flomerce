...
- Stale-translation detection (`diffStaleness` in `i18n-worker.js` + `hashCatalog` in `translator.js`) is verified end-to-end: editing any string in `en.json`, restarting the worker, and re-fetching `GET /api/admin/i18n/locales` correctly reports cached locales as out-of-date (via `stale` for changed strings and/or `added` for new keys, plus `upToDate=false`); reverting the edit and restarting flips them back to `upToDate=true` without spending any translator quota. The Owner Admin → Translations panel renders its orange banner from this same response (`I18nAdminPanel.jsx` line ~106). Refresh-all returns 200 with per-locale errors when translator credentials are missing rather than throwing. **Not yet verified in this environment:** the refresh-all success path (out-of-date → upToDate via actual Microsoft Translator round-trip) — that requires a credentialed environment with translator keys configured in Owner Admin → Settings.
