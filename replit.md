...
- **Frontend gating:** `FeatureGate` overlay component (`frontend/src/storefront/src/components/admin/FeatureGate.jsx`) wraps locked sections with upgrade prompt. `AdminSidebar` shows lock icons and plan badges. `VisualCustomizer` gates Blog and Customer Reviews sections. `SEOSection` gates Pages/Categories/Products SEO tabs. `DashboardPage` enforces trial site creation limit and staff count limits with disabled buttons.
