    ...
    <div style={{ minHeight: '80vh' }}>
        ...
        {activeTab === 'account' && (
          <div>
            <div style={{ marginBottom: 25 }}>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="Full Name" /></label>
              {editingName ? (
            ...
            <div style={{ marginBottom: 25 }}>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="Email" /></label>
          ...
          <div>
            {ordersLoading ? (
              <div style={{ textAlign: 'center', padding: 40 }}>
                ...
                <p style={{ color: '#777' }}><TranslatedText text="Loading orders..." /></p>
                <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
              </div>
            ) : orders.length === 0 ? (
              <div style={{ textAlign: 'center', color: '#777', margin: '30px 0', fontStyle: 'italic' }}>
                <p><TranslatedText text="No orders found yet." /></p>
                <Link to="/" style={{ display: 'inline-block', backgroundColor: '#c8a97e', color: '#fff', padding: '10px 20px', borderRadius: 4, textDecoration: 'none', marginTop: 15 }}><TranslatedText text="Start Shopping" /></Link>
              </div>
            ) : (
              orders.map(order => {
                  ...
                  <div key={order.id} style={{ border: '1px solid #eee', borderRadius: 5, padding: 15, marginBottom: 15 }}>
                    ...
                    {order.status === 'cancelled' && order.cancellation_reason && (
                      <div style={{ marginBottom: 12, padding: '8px 12px', background: '#fff5f5', borderInlineStart: '3px solid #e74c3c', borderRadius: 4, fontSize: 13, color: '#555' }}>
                        <strong style={{ color: '#e74c3c' }}><TranslatedText text="Cancellation Reason:" /></strong> {order.cancellation_reason}
                      </div>
                    )}
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            {isDelivered && (
                              <div style={{ marginTop: 6 }}>
                                {alreadyReviewed ? (
                                  <span style={{ fontSize: 12, color: '#27ae60' }}><TranslatedText text="★ Reviewed" /></span>
                                ) : (
                                  <Link
                    ...
                    <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #eee' }}>
                      ...
                      {openHelpOrderId === order.id && (
                        <div style={{ marginTop: 10, padding: '12px 14px', background: '#f8fafc', borderRadius: 6, border: '1px solid #e2e8f0', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                          ...
                          >
                            <i className="fas fa-truck" style={{ fontSize: 11 }} /> <TranslatedText text="Track Order" />
                          </Link>
                          {cancellationEnabled && ['pending', 'confirmed'].includes((order.status || '').toLowerCase()) && !cancelStatuses[order.id] && (
                            ...
                            >
                              <i className="fas fa-times-circle" style={{ fontSize: 11 }} /> <TranslatedText text="Cancel Order" />
                            </button>
                          )}
                          {returnsEnabled && (order.status || '').toLowerCase() === 'delivered' && !returnStatuses[order.id] && isWithinReturnWindow(order) && (
                            ...
                            >
                              <i className="fas fa-undo" style={{ fontSize: 11 }} /> <TranslatedText text="Return / Refund" />
                            </button>
                          )}
                          {returnsEnabled && (order.status || '').toLowerCase() === 'delivered' && !returnStatuses[order.id] && !isWithinReturnWindow(order) && (
                            <span style={{ fontSize: 12, color: '#94a3b8', padding: '6px 0' }}><TranslatedText text="Return window expired" /></span>
                          )}
                        </div>
                      )}
                    </div>
        ...
        {activeTab === 'addresses' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", margin: 0 }}><TranslatedText text="Saved Addresses" /></h3>
              <button onClick={() => { setEditAddress(null); setAddressForm({ label: 'Home', firstName: '', lastName: '', phone: '', houseNumber: '', roadName: '', city: '', state: '', pinCode: '', isDefault: false }); setAddressError(''); setAddressFieldErrors({}); setShowAddressModal(true); }} style={{ background: '#c8a97e', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 4, cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
                <TranslatedText text="+ Add Address" />
              </button>
            </div>
            {addressesLoading ? (
              <div style={{ textAlign: 'center', padding: 40 }}>
                <div style={{ width: 40, height: 40, border: '4px solid #f3f3f3', borderTop: '4px solid #c8a97e', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 16px' }} />
                <p style={{ color: '#777' }}><TranslatedText text="Loading addresses..." /></p>
                <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
              </div>
            ) : addresses.length === 0 ? (
              <p style={{ textAlign: 'center', color: '#777', fontStyle: 'italic', padding: 30 }}><TranslatedText text="No saved addresses. Add one for faster checkout." /></p>
            ) : (
              addresses.map((addr) => (
                <div key={addr.id} style={{ border: `1px solid ${addr.is_default ? '#c8a97e' : '#eee'}`, borderRadius: 8, padding: 20, marginBottom: 15, backgroundColor: addr.is_default ? '#fefcf8' : '#fff', transition: 'box-shadow 0.3s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingBottom: 10, borderBottom: '1px solid #eee' }}>
                    ...
                    <div style={{ display: 'flex', gap: 15 }}>
                      <button onClick={() => openEditAddress(addr)} style={{ background: 'none', border: 'none', color: '#c8a97e', cursor: 'pointer', fontSize: 14 }}><TranslatedText text="Edit" /></button>
                      <button onClick={() => handleDeleteAddress(addr.id)} style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', fontSize: 14 }}><TranslatedText text="Delete" /></button>
                    </div>
                  </div>
      ...
      {showLogoutConfirm && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', padding: 30, borderRadius: 10, maxWidth: 400, width: '90%', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
            <h3 style={{ marginBottom: 15, fontFamily: "'Playfair Display', serif" }}><TranslatedText text="Confirm Logout" /></h3>
            <p style={{ color: '#666', marginBottom: 24 }}><TranslatedText text="Are you sure you want to log out?" /></p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button onClick={handleLogout} style={{ background: '#c8a97e', color: '#fff', border: 'none', padding: '10px 24px', borderRadius: 4, cursor: 'pointer', fontWeight: 'bold' }}><TranslatedText text="Yes, Logout" /></button>
              <button onClick={() => setShowLogoutConfirm(false)} style={{ background: '#f8f9fa', color: '#333', border: '1px solid #ddd', padding: '10px 24px', borderRadius: 4, cursor: 'pointer' }}><TranslatedText text="Cancel" /></button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 30, width: '90%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 15, borderBottom: '1px solid #eee' }}>
              <h3 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontSize: 20 }}><TranslatedText text="Request Return" /></h3>
            ...
            <p style={{ color: '#64748b', fontSize: 14, marginBottom: 20 }}>
              <TranslatedText text="Order" /> <strong>#{returnModal.order_number || returnModal.id}</strong>
            </p>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 10, color: '#333', fontSize: 14 }}><TranslatedText text="Reason for return *" /></label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {RETURN_REASONS.map(reason => (
                  <label key={reason} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px', borderRadius: 6, border: `1.5px solid ${returnReason === reason ? '#c8a97e' : '#e0e0e0'}`, background: returnReason === reason ? '#fefcf8' : '#fafafa', transition: 'all 0.15s' }}>
                    ...
                    <span style={{ fontSize: 14, color: '#333', fontWeight: returnReason === reason ? 600 : 400 }}><TranslatedText text={reason} /></span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 6, color: '#333', fontSize: 14 }}>
                <TranslatedText text="Photos" /> {imageRequiredReasons.includes(returnReason) ? "*" : "(optional)"} {returnPhotos.length > 0 ? `(${returnPhotos.length}/5)` : ''}
              </label>
              {imageRequiredReasons.includes(returnReason) && (
                <p style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}><TranslatedText text="Upload photos showing the issue (required)." /></p>
              )}
      ...
      {cancelModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 30, width: '90%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 15, borderBottom: '1px solid #eee' }}>
              <h3 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontSize: 20 }}><TranslatedText text="Request Cancellation" /></h3>
              <button onClick={() => setCancelModal(null)} style={{ background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', color: '#999' }}>x</button>
            </div>
            <p style={{ color: '#64748b', fontSize: 14, marginBottom: 20 }}>
              <TranslatedText text="Order" /> <strong>#{cancelModal.order_number || cancelModal.id}</strong>
            </p>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 10, color: '#333', fontSize: 14 }}><TranslatedText text="Reason for cancellation *" /></label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {CANCEL_REASONS.map(reason => (
                  <label key={reason} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px', borderRadius: 6, border: `1.5px solid ${cancelReason === reason ? '#e53935' : '#e0e0e0'}`, background: cancelReason === reason ? '#fff5f5' : '#fafafa', transition: 'all 0.15s' }}>
                    ...
                    <span style={{ fontSize: 14, color: '#333', fontWeight: cancelReason === reason ? 600 : 400 }}><TranslatedText text={reason} /></span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: 8, padding: 30, width: '90%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'flex', gap: 15, marginBottom: 0 }}>
              <div style={{ flex: 1, marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="First Name *" /></label>
              ...
              <div style={{ flex: 1, marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="Last Name" /></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="Phone" /></label>
              <PhoneInput
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="House/Building Number *" /></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="Road / Area / Colony" /></label>
            ...
            <div style={{ display: 'flex', gap: 15, marginBottom: 0 }}>
              <div style={{ flex: 1, marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="PIN Code *" /></label>
                ...
                {pinValidating && <div style={{ color: '#c8a97e', fontSize: 12, marginTop: 4 }}><TranslatedText text="Validating PIN code..." /></div>}
                {addressFieldErrors.pinCode && <div style={{ color: '#e74c3c', fontSize: 12, marginTop: 4 }}>{addressFieldErrors.pinCode}</div>}
              </div>
              <div style={{ flex: 1, marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#333' }}><TranslatedText text="City *" /></label>
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginTop: 20, marginBottom: 20 }}>
              ...
              <label htmlFor="defaultAddr" style={{ color: '#333', fontSize: 14 }}><TranslatedText text="Set as default address" /></label>
            </div>
