...
export default function PrivacyPolicyPage() {
  ...
  const defaultIntro = `Welcome to ${brand}. We are committed to protecting your personal information and your right to privacy. This Privacy Policy explains how we collect, use, and share information about you when you use our services.`;
  ...
  const defaultSections = [
    ...
    {
      title: '9. Contact Us',
      content: `If you have any questions about this Privacy Policy, please contact us at ${email}${siteConfig?.phone ? ` or call us at ${siteConfig.phone}` : ''}.`,
    },
  ];
  ...
  const intro = hasCustomContent && privacyContent.intro
    ? privacyContent.intro.replace(/\{brand\}/g, brand).replace(/\{email\}/g, email).replace(/\{phone\}/g, siteConfig?.phone || '')
    : defaultIntro;

  const sections = hasCustomContent
    ? privacyContent.sections.map(s => ({
        title: s.title,
        content: s.content.replace(/\{brand\}/g, brand).replace(/\{email\}/g, email).replace(/\{phone\}/g, siteConfig?.phone || ''),
      }))
    : defaultSections;
