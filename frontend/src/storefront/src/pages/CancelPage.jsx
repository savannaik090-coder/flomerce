...
export default function CancelPage() {
  ...
  const statusLabels = { requested: 'Pending Review', approved: 'Approved (Cancelled)', rejected: 'Rejected' };
  ...
  return (
    <div style={{ minHeight: '80vh', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '40px 16px' }}>
      <div style={{ maxWidth: 520, width: '100%', background: '#fff', borderRadius: 12, boxShadow: '0 2px 20px rgba(0,0,0,0.08)', padding: 32 }}>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, margin: '0 0 8px', color: '#1e293b' }}>Cancel Order</h1>

        {mode === 'lookup' && !orderId && (
          <>
            <p style={{ color: '#64748b', fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>
              Enter your order number and email address to receive a cancellation link.
            </p>
        ...
        {orderId && checkingStatus && (
          <div style={{ textAlign: 'center', padding: 40 }}>
            ...
            <p style={{ color: '#777' }}>Checking cancellation status...</p>
        ...
        {orderId && !checkingStatus && existingCancel && (
          <div style={{ marginTop: 16 }}>
            <p style={{ color: '#64748b', fontSize: 14, marginBottom: 16 }}>
              Order <strong>#{existingCancel.order_number || orderId}</strong>
            </p>
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 20, textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: '#64748b', marginBottom: 8 }}>Cancellation Status</div>
        ...
        {orderId && !checkingStatus && !existingCancel && !submitted && (
          <div style={{ marginTop: 16 }}>
            <p style={{ color: '#64748b', fontSize: 14, marginBottom: 24 }}>
              Submit a cancellation request for order <strong>#{orderId}</strong>
            </p>
            {error && (
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 10, fontSize: 14, color: '#334155' }}>Reason for cancellation *</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {CANCEL_REASONS.map(reason => (
                  <label key={reason} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px', borderRadius: 6, border: `1.5px solid ${cancelReason === reason ? '#e53935' : '#e2e8f0'}`, background: cancelReason === reason ? '#fff5f5' : '#fff', transition: 'all 0.15s' }}>
                    ...
                    <span style={{ fontSize: 14, color: '#334155', fontWeight: cancelReason === reason ? 600 : 400 }}>{reason}</span>
                  </label>
                ))}
              </div>
            </div>
        ...
        {submitted && (
          <div style={{ marginTop: 16, textAlign: 'center', padding: 20 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, margin: '0 0 8px', color: '#166534' }}>Cancellation Request Submitted</h2>
            <p style={{ color: '#64748b', fontSize: 14, lineHeight: 1.6 }}>
              We've received your cancellation request. You'll receive an email update once it's reviewed by the store.
            </p>
          </div>
        )}
      </div>
