  ...
  async function loadOrder(id) {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setError(t('orderHelp.errors.notFound', 'Order not found. Please check your order number.'));
        setLooked(true);
      }
    } catch {
      setError(t('orderHelp.errors.loadFailed', 'Could not load order details.'));
      setLooked(true);
    } finally {
      setLoading(false);
    }
    ...
    <div style={{ minHeight: '80vh', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '40px 16px' }}>
      <div style={{ maxWidth: 560, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: '0 0 8px', color: '#0f172a', fontFamily: "'Playfair Display', serif" }}>{t('orderHelp.headings.title', 'Order Help')}</h1>
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>
            {orderId ? t('orderHelp.headings.helpQuestion', 'How can we help with your order?') : t('orderHelp.headings.subtitleEnter', 'Enter your order number to see your options')}
          </p>
        </div>
        ...
        {!orderId && (
          <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 20px rgba(0,0,0,0.08)', padding: 32 }}>
            <form onSubmit={handleLookup}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 8, fontSize: 14, color: '#334155' }}>{t('order.orderNumber', 'Order Number')}</label>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  type="text"
                  value={lookupOrderId}
                  onChange={e => setLookupOrderId(e.target.value)}
                  placeholder={t('orderHelp.form.placeholder', 'e.g. ORD-XXXX')}
                ...
                <button type="submit" style={{ padding: '12px 20px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                  {t('orderHelp.actions.findOrder', 'Find Order')}
                </button>
              </div>
            </form>
          </div>
        ...
        {orderId && loading && (
          <div style={{ textAlign: 'center', padding: 48 }}>
            ...
            <p style={{ color: '#64748b' }}>{t('orderHelp.loading', 'Loading order details...')}</p>
        ...
        {orderId && !loading && error && (
          <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: 24, textAlign: 'center' }}>
            ...
            <Link to="/order-track" style={{ color: '#0f172a', fontWeight: 600, fontSize: 14 }}>{t('orderHelp.actions.tryTracking', 'Try Order Tracking instead →')}</Link>
          </div>
        )}
          ...
          <div>
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 20, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  <div style={{ fontSize: 12, color: '#64748b', marginBottom: 2 }}>{t('account.orderRefLine', 'Order')}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Link to={`/order-track?orderId=${order.order_number}`} style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '18px 20px', textDecoration: 'none', transition: 'box-shadow 0.2s', color: 'inherit' }}>
                ...
                <div>
                  <div style={{ fontWeight: 600, fontSize: 15, color: '#0f172a' }}>{t('orderHelp.actions.trackTitle', 'Track This Order')}</div>
                  <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{t('orderHelp.actions.trackDesc', 'See the current status and shipping updates')}</div>
                </div>
              ...
              {canCancel && (
                <Link to={cancelHref} style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '1px solid #fecaca', borderRadius: 12, padding: '18px 20px', textDecoration: 'none', transition: 'box-shadow 0.2s', color: 'inherit' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 15, color: '#0f172a' }}>{t('account.cancelOrder', 'Cancel Order')}</div>
                    <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{t('orderHelp.actions.cancelDesc', 'Request a cancellation for this order')}</div>
                  </div>
                  <i className="fas fa-chevron-right" style={{ marginInlineStart: 'auto', color: '#cbd5e1' }} />
                </Link>
              )}
              ...
              {canReturn && (
                <Link to={returnHref} style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '18px 20px', textDecoration: 'none', transition: 'box-shadow 0.2s', color: 'inherit' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 15, color: '#0f172a' }}>{t('account.returnRefund', 'Return / Refund')}</div>
                    <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{t('orderHelp.actions.returnDesc', 'Request a return or refund for this order')}</div>
                  </div>
                  <i className="fas fa-chevron-right" style={{ marginInlineStart: 'auto', color: '#cbd5e1' }} />
                </Link>
              )}
              ...
              {status === 'cancelled' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '18px 20px' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 15, color: '#dc2626' }}>{t('order.cancelledTitle', 'Order Cancelled')}</div>
                    <div style={{ fontSize: 13, color: '#991b1b', marginTop: 2 }}>{t('orderHelp.status.alreadyCancelled', 'This order has already been cancelled')}</div>
                  </div>
                </div>
              )}

              {ownerEmail && (
                <a href={`mailto:${ownerEmail}?subject=${encodeURIComponent(t('orderHelp.email.subject', 'Help with Order #{{num}}', { num: order.order_number }))}`} style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '18px 20px', textDecoration: 'none', transition: 'box-shadow 0.2s', color: 'inherit' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 15, color: '#0f172a' }}>{t('orderHelp.actions.contactTitle', 'Contact the Store')}</div>
                    <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{t('orderHelp.actions.contactDesc', 'Send an email to {{email}}', { email: ownerEmail })}</div>
                  </div>
                  <i className="fas fa-chevron-right" style={{ marginInlineStart: 'auto', color: '#cbd5e1' }} />
                </a>
              )}
        ...
        <div style={{ textAlign: 'center', marginTop: 28 }}>
          <Link to="/order-track" style={{ fontSize: 13, color: '#64748b', textDecoration: 'underline' }}>{t('orderHelp.actions.trackDifferent', 'Track a different order')}</Link>
        </div>
      </div>
    </div>
  );
