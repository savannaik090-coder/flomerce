    ...
    <div style={{ minHeight: '80vh', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '40px 16px' }}>
      <div style={{ maxWidth: 560, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: '0 0 8px', color: '#0f172a', fontFamily: "'Playfair Display', serif" }}>Order Help</h1>
        ...
        {!orderId && (
          <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 20px rgba(0,0,0,0.08)', padding: 32 }}>
            <form onSubmit={handleLookup}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 8, fontSize: 14, color: '#334155' }}>Order Number</label>
        ...
        {orderId && !loading && error && (
          <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: 24, textAlign: 'center' }}>
            ...
            <Link to="/order-track" style={{ color: '#0f172a', fontWeight: 600, fontSize: 14 }}>Try Order Tracking instead →</Link>
          </div>
        )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {status === 'cancelled' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '18px 20px' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 15, color: '#dc2626' }}>Order Cancelled</div>
                    <div style={{ fontSize: 13, color: '#991b1b', marginTop: 2 }}>This order has already been cancelled</div>
                  </div>
                </div>
              )}

              {ownerEmail && (
                <a href={`mailto:${ownerEmail}?subject=Help with Order #${order.order_number}`} style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '18px 20px', textDecoration: 'none', transition: 'box-shadow 0.2s', color: 'inherit' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 28 }}>
          <Link to="/order-track" style={{ fontSize: 13, color: '#64748b', textDecoration: 'underline' }}>Track a different order</Link>
        </div>
      </div>
    </div>
  );
