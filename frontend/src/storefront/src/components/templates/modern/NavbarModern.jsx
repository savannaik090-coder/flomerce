                ...
                return (
                  <li key={link.id} className={`mn-sub-group${isExpanded ? ' mn-sub-group-open' : ''}`}>
                    <div className="mn-grouped-cat-header">
                      ...
                      <button className="mn-subcategory-toggle" onClick={(e) => toggleSubGroup(`grouped-${link.id}`, e)} aria-label={tx("Show subcategories")}>
    ...
    <header className={headerClass}>
      ...
      <nav className="mn-navbar">
        <div className={`mn-nav-container${!isCentered ? ' mn-logo-left' : ''}`}>
          ...
          <div className="mn-nav-icons">
            ...
            <button className="mn-icon-btn" onClick={() => onSearchOpen?.()} aria-label={tx("Search")}>
            ...
            {showAccountIcon && (
              <Link to={isAuthenticated ? '/profile' : '/login'} className="mn-icon-btn" aria-label={tx("Account")}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
            ...
            <button className="mn-icon-btn mn-icon-badge" onClick={() => onWishlistOpen?.()} aria-label={tx("Wishlist")}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
            ...
            {showCartIcon && (
              <button className="mn-icon-btn mn-icon-badge" onClick={() => onCartOpen?.()} aria-label={tx("Cart")}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
      ...
      <div className={`mn-mobile-menu${menuOpen ? ' mn-mobile-open' : ''}`}>
        <div className="mn-mobile-header">
          <span className="mn-mobile-title">{siteConfig?.brandName || tx("Menu")}</span>
          <button className="mn-mobile-close" onClick={closeMobileMenu} aria-label={tx("Close")}>
