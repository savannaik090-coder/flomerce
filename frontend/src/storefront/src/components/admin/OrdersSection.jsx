                        ...
                        <tr key={ret.id} onClick={() => setReturnDetailModal(ret)} style={{ cursor: 'pointer' }}>
                          ...
                          <td>
                            ...
                            <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                              ...
                              {(() => { try { const p = typeof ret.photos === 'string' ? JSON.parse(ret.photos) : ret.photos; return p && p.length > 0 ? <span style={{ fontSize: 11, color: '#64748b' }}><i className="fas fa-camera" style={{ fontSize: 10 }} /> {(p.length === 1 ? `${p.length} photo` : `${p.length} photos`)}</span> : null; } catch { return null; } })()}
                            </div>
                          </td>
                          <td>
            ...
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
              <div style={{ background: '#fff', borderRadius: 10, padding: 28, width: '90%', maxWidth: 440, boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
                <h3 style={{ margin: '0 0 6px', fontSize: 18, color: '#1a1a1a' }}>
                  {returnAction === 'approved' ? "Approve Return" : returnAction === 'rejected' ? "Reject Return" : "Process Refund for Return"}
                </h3>
                <p style={{ margin: '0 0 20px', fontSize: 14, color: '#666' }}>
                  {`Order #${returnModal.order_number} - ${returnModal.customer_name}`}
                </p>
        ...
        <div>
          ...
          {cancellationModal && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
              <div style={{ background: '#fff', borderRadius: 10, padding: 28, width: '90%', maxWidth: 440, boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
                <h3 style={{ margin: '0 0 6px', fontSize: 18, color: '#1a1a1a' }}>
                  {cancellationAction === 'approved' ? "Approve Cancellation" : "Reject Cancellation"}
                </h3>
                <p style={{ margin: '0 0 20px', fontSize: 14, color: '#666' }}>
                  {`Order #${cancellationModal.order_number} - ${cancellationModal.customer_name}`}
                </p>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', fontWeight: 600, marginBottom: 6, fontSize: 13 }}>Reason from customer</label>
                  <div style={{ padding: '10px 12px', background: '#f8fafc', borderRadius: 6, fontSize: 13, color: '#334155', border: '1px solid #e2e8f0' }}>
                  ...
                  {filtered.map(order => {
                    ...
                    return (
                        <tr key={order.id} onClick={() => openOrderDetail(order)} style={{ cursor: 'pointer' }}>
                          ...
                          <td>{items.length > 0 ? (items.length === 1 ? `${items.length} item` : `${items.length} items`) : '—'}</td>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              ...
              >
                {confirming ? (
                  ...
                  confirmDialog.action === 'delivered' ? "Yes, Mark Delivered" : "Yes, Confirm Order"
                )}
              </button>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              {CANCEL_REASON_KEYS.map(({ value: reason, key }) => (
                <label key={reason} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px', borderRadius: 6, border: `1.5px solid ${cancelReason === reason ? '#e53935' : '#e0e0e0'}`, background: cancelReason === reason ? '#fff5f5' : '#fafafa', transition: 'all 0.15s' }}>
                  ...
                  />
                  <span style={{ fontSize: 14, color: '#333', fontWeight: cancelReason === reason ? 600 : 400 }}>{({ outOfStock: 'Item out of stock', customerRequested: 'Customer requested cancellation', duplicate: 'Duplicate order', fraud: 'Suspected fraud', other: 'Other' })[key] || key}</span>
                </label>
              ))}
            </div>
