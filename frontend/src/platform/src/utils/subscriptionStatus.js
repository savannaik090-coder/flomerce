...
/**
 * Returns a plan label, or null if the plan is one of the "non-plan"
 * values (free / expired / paused / cancelled) or unknown.
 */
...
export function getSubscriptionBadge(info) {
  ...
  if (sub.isCancelled && sub.isActive) {
    return {
      text: planName ? `${planName} - Cancelling` : 'Cancelling',
      bg: '#fef3c7',
      color: '#92400e',
      tone: 'cancelling',
    };
  ...
  if (sub.isActive) {
    ...
    return {
      text: planName ? `${planName} - Active` : 'Active',
      bg: '#dcfce7',
      color: '#166534',
      tone: 'active',
    };
  ...
  if (sub.isExpired || sub.status === 'expired' || sub.status === 'cancelled' || sub.status === 'paused') {
    return {
      text: planName ? `${planName} - Expired` : 'Expired',
      bg: '#fee2e2',
      color: '#dc2626',
      tone: 'expired',
    };
...
/**
 * Returns the badge text. Kept as a separate helper so existing callers
 * that pass `(badge, t)` keep compiling — the second argument is ignored.
 */
export function renderBadgeText(badge) {
  return badge?.text || '';
}
