...
async function createOrder(request, env, user, ctx) {
  try {
    ...
    await db.prepare(
      `INSERT INTO orders (id, site_id, user_id, order_number, items, subtotal, discount, shipping_cost, tax, total, currency, payment_method, status, shipping_address, billing_address, customer_name, customer_email, customer_phone, coupon_code, notes, whatsapp_opted_in, placed_in_language, row_size_bytes, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`
    ).bind(
      orderId,
      siteId,
    ...
    if (!isPendingPayment) {
      ...
      // Auto-ship for COD orders when merchant has the toggle on. Fire-and-
      // forget via ctx.waitUntil so we never block the order success response.
      try {
        const isCodOrder = (paymentMethod || '').toLowerCase() === 'cod' || (paymentMethod || '').toLowerCase() === 'cash_on_delivery';
        if (isCodOrder) {
          const cfgForShip = await getSiteConfig(env, siteId);
          let s = {};
          try { if (cfgForShip.settings) s = typeof cfgForShip.settings === 'string' ? JSON.parse(cfgForShip.settings) : cfgForShip.settings; } catch {}
          if (s?.shiprocket?.enabled && s?.shiprocket?.autoShipOnConfirmCod) {
            const shipPromise = (async () => {
              try {
                const { shipOrderViaShiprocket } = await import('./shipping-worker.js');
                const r = await shipOrderViaShiprocket(env, siteId, orderId);
                if (!r?.ok) console.warn('[auto-ship COD] failed:', r);
              } catch (e) { console.error('[auto-ship COD] error:', e); }
            })();
            if (ctx?.waitUntil) ctx.waitUntil(shipPromise); else await shipPromise;
          }
        }
      } catch (e) { console.error('Auto-ship-on-confirm-cod hook error:', e); }
    }
  ...
  try {
    ...
    if (status) {
      ...
      if (status === 'confirmed') {
        ...
        try { await db.prepare(`ALTER TABLE ${tableName} ADD COLUMN confirmed_at TEXT`).run(); } catch {}
      } else if (status === 'packed') {
        updates.push('packed_at = datetime("now")');
        try { await db.prepare(`ALTER TABLE ${tableName} ADD COLUMN packed_at TEXT`).run(); } catch {}
    ...
    if (status === 'cancelled') {
      try {
        ...
        if (fullOrder) {
          ...
          const cancelSiteId = fullOrder.site_id;
          ...
          if (isWhatsAppConfigured(cancelSettings) && fullOrder.customer_phone && fullOrder.whatsapp_opted_in) {
            try {
              const waMsg = await buildOrderCancelledWA(emailOrder, siteBrandName, cancellationReason, cancelCurrency, env, cancelSiteId, placedLang);
        ...
        if (fullOrder) {
          ...
          if (fullOrder.customer_email) {
            ...
            if (status === 'confirmed' && statusSettings.cancellationEnabled) {
              try {
                ...
                try { await db.prepare(`ALTER TABLE ${tableName} ADD COLUMN cancel_token TEXT`).run(); } catch (e) {}
          ...
          if (isWhatsAppConfigured(statusSettings) && fullOrder.customer_phone && fullOrder.whatsapp_opted_in) {
            try {
              ...
              let waMsg;
              if (status === 'confirmed') {
                waMsg = await buildOrderConfirmationWA(emailOrder, siteBrandName, statusCurrency, env, statusSiteId, placedLang);
              } else if (status === 'packed') {
                waMsg = await buildOrderPackedWA(emailOrder, siteBrandName, env, statusSiteId, placedLang);
              } else if (status === 'shipped') {
                waMsg = await buildOrderShippedWA(emailOrder, siteBrandName, trackingUrl, statusCurrency, env, statusSiteId, placedLang);
              }
    ...
    if (status === 'delivered') {
      try {
        ...
        if (fullOrder) {
          ...
          const placedLang = fullOrder.placed_in_language || null;
          const deliverySiteId = fullOrder.site_id;
          const emailJobs = [];
          if (fullOrder.customer_email) {
            try {
          ...
          if (isWhatsAppConfigured(deliverySettings) && fullOrder.customer_phone && fullOrder.whatsapp_opted_in) {
            try {
              const waMsg = await buildOrderDeliveredWA(emailOrder, siteBrandName, deliveryEmailOptions.reviewUrl || '', deliveryCurrency, env, deliverySiteId, placedLang);
...
// sendShiprocketStatusNotification has moved to backend/utils/shiprocket-notifications.js
// to break the circular import between this file and shipping-worker.js.
  ...
  try {
    ...
    if (status === 'rejected' && req.customer_email) {
      try {
        ...
        let placedLang = null;
        try {
          const orderTbl = req.order_type === 'guest_order' ? 'guest_orders' : 'orders';
          const ord = await db.prepare(`SELECT placed_in_language FROM ${orderTbl} WHERE id = ?`).bind(req.order_id).first();
          placedLang = ord?.placed_in_language || null;
        } catch (e) {}
