...
import { resolveEnterpriseLimits, loadOverageRates } from './admin-worker.js';
import { getSiteUsage } from '../../utils/usage-tracker.js';
...
async function listInvoicesForSite(request, env) {
  ...
  // Also surface the merchant's CURRENT effective quota and live usage so the
  // Billing tab reflects platform-side overrides immediately (without waiting
  // for the next month-end snapshot). Returns null `quota` for non-enterprise
  // sites so the UI can hide the block gracefully.
  let quota = null;
  try {
    ...
    // Quota lookup is best-effort — invoices list still loads.
