...
// .wrangler/tmp/bundle-fpuL3E/checked-fetch.js
...
// .wrangler/tmp/bundle-fpuL3E/strip-cf-connecting-ip-header.js
...
function getAllowedOrigin(origin) {
  ...
  if (origin.endsWith(`.${PLATFORM_DOMAIN}`) && origin.startsWith("https://"))
    return origin;
  if (origin === "https://flomerce.pages.dev")
    return origin;
...
__export(email_exports, {
  buildAbandonedCartEmail: () => buildAbandonedCartEmail,
  buildCancellationCustomerEmail: () => buildCancellationCustomerEmail,
  buildCancellationOwnerEmail: () => buildCancellationOwnerEmail,
  buildCancellationRequestNotifyEmail: () => buildCancellationRequestNotifyEmail,
  buildCancellationStatusEmail: () => buildCancellationStatusEmail,
  buildDeliveryCustomerEmail: () => buildDeliveryCustomerEmail,
  buildDeliveryOwnerEmail: () => buildDeliveryOwnerEmail,
  buildNewOrderReviewEmail: () => buildNewOrderReviewEmail,
  buildOrderConfirmationEmail: () => buildOrderConfirmationEmail,
  buildOrderPackedEmail: () => buildOrderPackedEmail,
  buildOrderShippedEmail: () => buildOrderShippedEmail,
  buildOwnerNotificationEmail: () => buildOwnerNotificationEmail,
  getOwnerRecipients: () => getOwnerRecipients,
  sendEmail: () => sendEmail
});
...
function getOwnerRecipients(settings = {}, config = {}) {
  ...
  const out = [];
  const addEmail = /* @__PURE__ */ __name((raw) => {
  ...
  addEmail(primary);
  for (const cc of ccRaw.slice(0, 5))
    addEmail(cc);
  return out;
}
...
function buildOwnerNotificationEmail(order, brandName, currency = "INR", timezone = "") {
  const items = typeof order.items === "string" ? JSON.parse(order.items) : order.items;
  const itemsHtml = items.map((item) => `
    <tr>
      <td style="padding: 10px 16px; border-bottom: 1px solid #f0f0f0; font-size: 14px;">${item.name}${formatSelectedOptions(item.selectedOptions, currency)}</td>
...
function buildAbandonedCartEmail(customerName, brandName, items, cartTotal, storeUrl, currency = "INR") {
  const fmtH = /* @__PURE__ */ __name((amt) => formatCurrencyHtml(amt, currency), "fmtH");
  ...
  const html = `
    ...
    <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Arial, sans-serif; background: #f5f5f5;">
      <div style="max-width: 600px; margin: 0 auto; background: #ffffff;">
        ...
        <div style="padding: 32px;">
          <div style="text-align: center; margin-bottom: 24px;">
            <div style="font-size: 48px; margin-bottom: 12px;">\u{1F6D2}</div>
...
var init_email = __esm({
  "utils/email.js"() {
    ...
    __name(getOwnerRecipients, "getOwnerRecipients");
    ...
    __name(buildAbandonedCartEmail, "buildAbandonedCartEmail");
  }
});
...
var init_notifications_worker = __esm({
  "workers/storefront/notifications-worker.js"() {
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
...
var init_products_worker = __esm({
  "workers/storefront/products-worker.js"() {
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
...
// workers/storefront/inventory-locations-worker.js
...
async function handleInventoryLocations(request, env, path, ctx) {
  ...
  if (segment === "transfers") {
    if (method === "GET")
      return getTransfers(db, siteId, url);
    if (method === "POST")
      return createTransfer(request, db, siteId, admin);
    return errorResponse("Method not allowed", 405);
  }
  if (segment === "levels") {
    if (method === "GET")
      return getLevels(db, siteId, url);
    if (method === "PUT")
      return setLevel(request, db, siteId);
    return errorResponse("Method not allowed", 405);
  }
  if (segment === "sync") {
    if (method === "POST")
      return syncProductStock(db, siteId, url);
    return errorResponse("Method not allowed", 405);
  }
  if (method === "GET")
  ...
  if (method === "DELETE" && segment)
    return deleteLocation(db, siteId, segment);
  return errorResponse("Method not allowed", 405);
}
...
async function updateLocation(request, db, siteId, locationId) {
  try {
    ...
    await db.prepare(
      ...
      address !== void 0 ? address : existing.address,
      priority !== void 0 ? priority : existing.priority,
...
async function setLevel(request, db, siteId) {
  try {
    ...
    if (!product_id || !location_id)
      return errorResponse("product_id and location_id are required", 400);
    if (stock === void 0 || stock === null || stock < 0)
      return errorResponse("Stock must be a non-negative number", 400);
    const loc = await db.prepare(
...
async function createTransfer(request, db, siteId, user) {
  try {
    ...
    if (from_location_id === to_location_id)
      return errorResponse("Source and destination must be different", 400);
...
async function deductStockByLocation(db, siteId, productId, quantity) {
  try {
...
var init_inventory_locations_worker = __esm({
  "workers/storefront/inventory-locations-worker.js"() {
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
    ...
    __name(handleInventoryLocations, "handleInventoryLocations");
    ...
    __name(syncProductStockFromLevels, "syncProductStockFromLevels");
    __name(syncProductStock, "syncProductStock");
...
__export(whatsapp_exports, {
  buildAbandonedCartWA: () => buildAbandonedCartWA,
  buildOrderCancelledWA: () => buildOrderCancelledWA,
  buildOrderConfirmationWA: () => buildOrderConfirmationWA,
  buildOrderDeliveredWA: () => buildOrderDeliveredWA,
  buildOrderPackedWA: () => buildOrderPackedWA,
  buildOrderShippedWA: () => buildOrderShippedWA,
...
function buildAbandonedCartWA(customerName, brandName, itemsSummary, cartTotal, storeUrl, currency = "INR") {
  const total = formatCurrency2(cartTotal, currency);
  const text = `\u{1F6D2} *Don't forget your cart!*
...
\u{1F4B0} *Cart Total:* ${total}

\u{1F449} Complete your purchase now: ${storeUrl}
...
var init_whatsapp = __esm({
  "utils/whatsapp.js"() {
    ...
    __name(buildAbandonedCartWA, "buildAbandonedCartWA");
    ...
    const emailJobs = [];
    {
      ...
      if (ownerRecipients.length) {
        try {
          ...
          emailJobs.push(
            sendEmail(env, ownerRecipients, `New Order #${orderData.orderNumber} - Review Required - ${siteBrandName}`, html, text, { senderName: siteBrandName }).catch((e) => console.error("Owner email send error:", e))
          );
        } catch (buildErr) {
...
var init_orders_worker = __esm({
  "workers/storefront/orders-worker.js"() {
    ...
    init_helpers();
    init_auth();
    init_products_worker();
    init_inventory_locations_worker();
    init_email();
    init_whatsapp();
    init_usage_tracker();
    init_site_db();
...
async function createRazorpaySubscription(request, env, user) {
  try {
    ...
    if (scheduledStartAt)
      subBody.start_at = scheduledStartAt;
...
async function handleSubscriptionCharged(env, subEntity, paymentEntity) {
  ...
  try {
    ...
    if (existingSub) {
      ...
      if (existingEnd && Math.abs(existingEnd - newEnd.getTime()) < 6e4) {
...
async function handleSubscriptionCancelled(env, entity) {
  ...
  try {
    ...
    if (sub) {
      ...
      const periodEndCovered = sub.current_period_end && new Date(sub.current_period_end) > /* @__PURE__ */ new Date();
      if (sub.site_id) {
...
async function activateScheduledSubscription(env, sub) {
  try {
    await env.DB.prepare(
...
async function activateSubscription(env, userId, planName, billingCycle, razorpayPaymentId, razorpaySubscriptionId, amount, siteId, razorpayEntity) {
  try {
    ...
    if (razorpayEntity?.current_start && razorpayEntity?.current_end) {
      ...
      periodStart = new Date(razorpayEntity.start_at * 1e3);
    ...
    const now = Date.now();
    const isScheduledStart = periodStart.getTime() > now + 6e4;
...
var init_payments_worker = __esm({
  "workers/platform/payments-worker.js"() {
    init_checked_fetch();
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
    ...
    __name(fetchRazorpaySubscriptionEntity, "fetchRazorpaySubscriptionEntity");
    ...
    __name(activateScheduledSubscription, "activateScheduledSubscription");
...
var init_admin_worker = __esm({
  "workers/platform/admin-worker.js"() {
    ...
    AUTOINCREMENT_TABLES = /* @__PURE__ */ new Set(["page_views", "site_media"]);
...
async function checkUsageLimit(env, siteId, resourceType = "d1", additionalBytes = 0) {
  try {
    ...
    if (isEnterpriseSite) {
      ...
      const rates = await getOverageRates2(env);
...
// .wrangler/tmp/bundle-fpuL3E/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-fpuL3E/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
      ...
      try {
        ...
        while (scanned < MAX_KEYS_PER_SITE && orphans.length < MAX_DELETES_PER_SITE) {
          ...
          for (const obj of listing.objects || []) {
            ...
            if (!Number.isFinite(uploadedMs) || uploadedMs <= 0)
              continue;
            if (uploadedMs > cutoff)
              continue;
            orphans.push(obj.key);
            if (orphans.length >= MAX_DELETES_PER_SITE)
              break;
          }
...
async function processAbandonedCartReminders(env) {
  ...
  const { buildAbandonedCartEmail: buildAbandonedCartEmail2 } = await Promise.resolve().then(() => (init_email(), email_exports));
  const { sendOrderWhatsApp: sendOrderWhatsApp2, buildAbandonedCartWA: buildAbandonedCartWA2, isWhatsAppConfigured: isWhatsAppConfigured2 } = await Promise.resolve().then(() => (init_whatsapp(), whatsapp_exports));
  try {
    ...
    for (const site of allSites.results || []) {
      try {
        ...
        if (!abandonedCarts.results || abandonedCarts.results.length === 0)
          continue;
        const brandName = site.brand_name || "Store";
        const domain = env.DOMAIN || PLATFORM_DOMAIN;
        const storeUrl = site.custom_domain && site.domain_status === "verified" ? `https://${site.custom_domain}` : `https://${site.subdomain}.${domain}`;
        ...
        for (const cart of abandonedCarts.results) {
          try {
            ...
            if (currentReminderCount > 0 && cart.reminder_sent_at) {
              ...
              const nextSendTime = new Date(new Date(cart.reminder_sent_at).getTime() + nextDelay * 60 * 60 * 1e3);
              if (/* @__PURE__ */ new Date() < nextSendTime)
                continue;
            }
            ...
            const cartTotal = cart.subtotal || enrichedItems.reduce((sum, item) => sum + Number(item.price) * Number(item.quantity), 0);
            const itemsSummary = enrichedItems.slice(0, 5).map(
            ...
            if (sendEmailChannel && customer.email) {
              try {
                const emailContent = buildAbandonedCartEmail2(
                  customer.name,
                  brandName,
                  enrichedItems,
                  cartTotal,
            ...
            if (sendWhatsApp && customer.phone && isWhatsAppConfigured2(settings)) {
              try {
                const waMessage = buildAbandonedCartWA2(
                  customer.name,
                  brandName,
                  itemsSummary,
                  cartTotal,
                ...
                const result = await sendOrderWhatsApp2(settings, customer.phone, waMessage);
...
__name(processAbandonedCartReminders, "processAbandonedCartReminders");
