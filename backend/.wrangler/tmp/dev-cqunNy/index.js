...
// .wrangler/tmp/bundle-Lf2sky/checked-fetch.js
...
// .wrangler/tmp/bundle-Lf2sky/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-Lf2sky/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Lf2sky/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function createCategory(request, env, user) {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO categories (id, site_id, name, slug, description, parent_id, image_url, display_order, subtitle, show_on_home, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`
    ).bind(
      categoryId,
      siteId,
...
async function updateCategory(request, env, user, categoryId) {
  ...
  try {
    ...
    const updates = await request.json();
    const allowedFields = ["name", "description", "parent_id", "image_url", "display_order", "is_active", "subtitle", "show_on_home"];
    const setClause = [];
    const values = [];
    for (const [key, value] of Object.entries(updates)) {
      const dbKey = key.replace(/([A-Z])/g, "_$1").toLowerCase();
