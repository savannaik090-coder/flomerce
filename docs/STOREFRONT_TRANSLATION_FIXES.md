...
- Phase 7 — DONE: ProductReviews sort dropdown + every shopper page now wraps every visible literal. Buttons that toggled labels via `{cond ? "X" : "Y"}` (Place Order/Processing, Login/Logging in, Save/Saving, Submit Return/Submitting, Reset Password, Hide/Show, Tracking labels, modal headings, etc.) now render `<TranslatedText>` on each branch. JSX `placeholder=`, `aria-label`, `alt`, `title` literals route through `tx("...")` (the runtime hook backed by the same server dictionary). Address modal `<option>` labels (Home / Work / Other / Select State) use `tx()`. CheckoutPage step 2 PIN/Postal label and step 3 (Phone:/Email:, Subtotal, Coupon, Free, Free shipping above) interpolate via `<TranslatedText vars={{...}}>`.
- Phase 7 — Extractor (`scripts/extract-i18n.cjs`) extended in this round: added `JSX_ATTR_RE` (placeholder/aria-label/alt/title in JSX, both `="..."` and `={"..."}` forms), `TX_CALL_RE` (`tx(...)` / `translateString(...)` calls — picked up across all files, not just `defaults/`), and `CONST_ARRAY_RE` (scrapes string entries from `CANCEL_REASONS` / `RETURN_REASONS` / `STATUS_LABELS` / `STEP_LABELS` arrays so radio labels rendered via `<TranslatedText text={reason} />` get into the dictionary). `OrderTrackPage` `stepLabels`/`statusLabels` and `BookAppointmentPage` `PURPOSES` now build their values through `tx("...")` so the literals are extractor-visible.
- Phase 8 — DONE: extraction regenerated; **manifest now 786 strings (was 508), hash `2a2dd441684f45c7`** (707 chrome literals, 233 JSX attrs, 88 `tx()` calls, 24 const-array entries, 118 defaults values). Validation error literals in `CheckoutPage.validateAddress` / `applyCoupon`, `ProfilePage.validateAddressForm`, and `SignupPage.validate` are now wrapped in `tx("...")` (architect HIGH addressed); the `ProfilePage.getStatusLabel` map values also route through `tx()` (architect MEDIUM addressed). Coupon-min-order error uses `tx("Minimum order amount for this coupon is {{amount}}").replace('{{amount}}', formatAmount(found.minOrder))`. Storefront `npm run build` passes. Backend restarted to pick up the new manifest.

Out of scope by design and intentionally left in source English: legal page bodies (Terms/Privacy/Refund/Shipping), `brand_name` literals, phone/email contact strings, owner/admin emails, the platform `AdminPanel` login screen, and the `ShopperTranslationContext` `FullPageLoader` aria-label (rendered before the dictionary is available).
